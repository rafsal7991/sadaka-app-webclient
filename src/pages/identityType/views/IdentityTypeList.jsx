...
const IdentityTypeList = () => {
  ...
  useEffect(() => {
    pullIdentiesFromServer();
    return () => {
    };
  }, []);
  async function pullIdentiesFromServer(){
    setIsDataLoading(true);
    const identityResponseWrapper= await IdentityController.getIdentityTypes(user.accessToken);
    if(identityResponseWrapper.error){
      if(identityResponseWrapper.status==401){
        dispatch(addAlert("Logout Success", "success", "Session Timeout."));
        dispatch(logout());
       }
       if(identityResponseWrapper.status==404){
         setData([]);
       }
    }
    else{
      const IdentityTypeDetails=identityResponseWrapper.resultCurl.IdTypResp.IdTypDtl;
      setData(IdentityTypeDetails.map((e,index)=>{
        return {
          sNo:index+1,
          IdTyp:e.IdTyp,
          code:e.IdTypCode,
  ...
  return (
    <>
      ...
      <div className="md:flex justify-between items-center mb-6">
        <h4 className="card-title">Identity type List</h4>
        <div className="flex space-x-2">
          <button
            className="btn btn-primary flex items-center  gap-2"
            onClick={() => {navigate('/settings/addIdentity')}}
          >
          <PlusCircle size={15}/>  Add Identity
          </button>
        </div>
      </div>
...
export default IdentityTypeList;
