...
import { churchLeaders } from "../../../mocks/churchLeaders"; // Import churchLeaders data
...
const UserTablePage = () => {
  const COLUMNS = [
...

    {
      ...
      accessor: "role",
      Cell: (row) => {
        const userRole = churchLeaders.find(
          (leader) => leader.name === row?.cell?.value
        );
        return (
          <span className="block w-full">
            <Badge
              label={userRole?.title} // Display the role title from churchLeaders data
              className={clsx("bg-opacity-10 rounded-full ", {
                "bg-fuchsia-500 text-fuchsia-500":
                  userRole?.role === "superadmin",
                "bg-green-500 text-green-500": userRole?.role === "admin",
                "bg-[#13317d]t-indigo-500": userRole?.role === "author",
              })}
            />
          </span>
        );
            ...
            >
              <div className="divide-y divide-gray-100 dark:divide-gray-800">
                {actions.map((item, i) => (
                 ...
                 hover:bg-[#13317d]/10  hover:text-indigo-500
