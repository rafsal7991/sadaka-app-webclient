...
const DeviceModalList = () => {
  ...
  async function getDeviceModelList() {
    ...
    const result = await DeviceModelController.getDeviceModels(
    ...
    if (result.error) {
      ...
      const deviceDtl = deviceResponseWrapper.DevModResp.DevModDtl.map(
        (e, index) => {
          return {
            sNo: index + 1,
            ApvdBy: e.ApvdBy,
        ...
        Cell: (row) => {
          ...
          ["MANAGE_DEVICE_MODEL", "VIEW_DEVICE_MODEL"],
            GlobalMethod.getUserPermissionName(permission)
              ...
              onClick={() =>{ 
                navigate("/settings/viewDeviceModel" ,{
          ...
          ["MANAGE_DEVICE_MODEL", "EDIT_DEVICE_MODEL"],
            GlobalMethod.getUserPermissionName(permission)
              ...
              onClick={() =>{ 
                navigate("/settings/EditDeviceModel",{
                    state: { row: row?.cell?.row?.original},
                  })
                }}
              >
  ...
  return (
    <>
      ...
      <div className="md:flex justify-between items-center mb-6">
        <h4 className="card-title">Device Modal List</h4>
        ...
        {GlobalMethod.hasAnyPermission(
          ...
          <button
            ...
            onClick={() => {navigate('/settings/addDeviceModel')}}
          >
          <PlusCircle size={15}/>  Add DeviceModal
          </button>)}
        </div>
      </div>
