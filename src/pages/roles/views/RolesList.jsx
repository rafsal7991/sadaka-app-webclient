...
const CustomerList = () => {
  ...
  async function pullRoleFromServer() {
    setIsLoading(true);
    const roleResponseWrapper = await RoleController.getAllRoles(user.accessToken);
    if (roleResponseWrapper.error) {
      if (roleResponseWrapper.status == 401) {
        dispatch(addAlert("Logout Success", "success", "Session Timeout."));
        dispatch(logout());
      }
      if (roleResponseWrapper.status == 404) {
        setData([]);
      }
    } else {
      const RoleDtl = roleResponseWrapper.resultCurl.RoleResp.RoleDtl;
      setData(
        RoleDtl.map((e, index) => {
          return {
            sNo: index + 1,
      ...
      Cell: (row) => {
       ...
       ["MANAGE_ROLE", "CREATE_ROLE"],
          GlobalMethod.getUserPermissionName(permission)
            ...
            onClick={() =>{ 
              navigate("/userManagement/EditRole",{
                  state: { row: row?.cell?.row?.original},
                })
              }}
            >
  ...
  return (
    ...
    <div className="md:flex justify-between items-center mb-6">
      <h4 className="card-title mt-2">Roles</h4>
      <div className="flex space-x-2">
        <button
          className="btn btn-primary flex gap-2"
          onClick={() => { navigate('/userManagement/addRole'); }}
        >
