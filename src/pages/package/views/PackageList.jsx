...
const PackageList = () => {
  ...
  useEffect(() => {
    pullPackageFromServer();
    return () => {};
  }, []);

  async function pullPackageFromServer(){
    setIsDataLoading(true);
    const packageResponseWrapper= await  PackageController.getPackagesByspRefNum(user.userInfo.UsrResp.UsrDtl,user.accessToken);

    if(packageResponseWrapper.error){
      if(packageResponseWrapper.status==401){
        dispatch(addAlert("Logout Success", "success", "Session Timeout."));
        dispatch(logout());
       }
       if(packageResponseWrapper.status==404){
         setData([]);
       }
    }
    else{
      const PackageDtl=packageResponseWrapper.resultCurl.PgResp.PgDtl;
      setData(PackageDtl.map((e,index)=>{
        return {
          sNo:index+1,
          name:e.PgNm,
          description:e.PgDesc,
          code:e.PgCode,
          amount:e.PgAmt,
          PgStDt:e.PgStDt,
          PgEndDt:e.PgEndDt,
          pinNumber:e.SpRefNum,
          branchId:e.SubSpRefNum,
          DevId:e.DevId,
          DevCode:e.DevCode,
          GenBy:e.GenBy,
          GenDt:e.GenDt,
      ...
      <div className="md:flex justify-between items-center mb-6">
        ...
        <div className="flex space-x-2">
          ...
          <button
            ...
            onClick={() => {navigate('/settings/AddPackages')}}
          >
