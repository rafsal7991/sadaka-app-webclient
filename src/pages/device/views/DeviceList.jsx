...
const DeviceList = () => {
  ...
  async function getDevices() {
    ...
    const result = await DeviceController.getDeviceBySpRefNum(
    ...
    if (result.status == 401) {
     ...
     if(result.status==200) {
      const deviceResponseWrapper = result.resultCurl;
      const supplDtl = deviceResponseWrapper.DevResp.DevDtl.map((e,index) => {
        return {
          sNo: index + 1,
          spName: e.SpNm,
          DevId1:e.DevId1,
          ...
          DevId2:e.DevId2,
          DevId3:e.DevId3,
          DevMdlCode:e.DevMdlCode,
          DevNum:e.DevNum,
          branch: e.SubSpRefNum,
          SpRefNum:e.SpRefNum,
          ApvdBy:e.ApvdBy,
          ApvdDt:e.ApvdDt,
          GenBy:e.GenBy,
        ...
        Cell: (row) => {
         ...
         ["MANAGE_DEVICE", "EDIT_DEVICE"],
            GlobalMethod.getUserPermissionName(permission)
              ...
              <button style={{ backgroundColor: '#E49B18' }} className="btn btn-secondary flex gap-2" type="button"
              onClick={() =>{ 
                navigate("/settings/editDevice",{
                    state: { row: row?.cell?.row?.original},
                  })
                }}
              >
      ...
      <div className="md:flex justify-between items-center mb-6">
        ...
        <div className="flex space-x-2">
          ...
          <button
            ...
            onClick={() => {navigate('/settings/addDevice')}}
          >
          <PlusCircle size={15}/>  Add Device
          </button>
        </div>
      </div>
